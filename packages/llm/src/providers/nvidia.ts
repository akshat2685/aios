...
export class NvidiaProvider implements ILLMProvider {
  public readonly id: LLMProviderId = 'nvidia';
  ...
  async generate(request: LLMRequest): Promise<LLMResponse> {
    try {
      ...
      const response = await this.client.post('/chat/completions', {
        model: request.model || 'meta/llama3-8b-instruct',
        messages,
      ...
      this.logger.error(`NVIDIA generate error: ${msg}`);
      throw new Error(`NVIDIA generate failed: ${msg}`);
    }
  }
  ...
  async stream(request: LLMRequest): Promise<AsyncGenerator<LLMStreamResponse>> {
    ...
    async function* run() {
      try {
          ...
          for (const line of lines) {
            ...
            if (cleanLine.startsWith('data: ')) {
              ...
              try {
                ...
                self.logger.warn(`Failed to parse NVIDIA stream chunk: ${e.message}`);
              }
            }
          }
        }
        ...
        const msg = error.response?.data?.error?.message || error.message;
        self.logger.error(`NVIDIA stream error: ${msg}`);
        throw new Error(`NVIDIA stream failed: ${msg}`);
      }
    }
    return run();
  }
